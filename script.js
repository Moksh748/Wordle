...
const index=wordList.indexOf(wordList[random]);
if (index<=1){
    wordList.splice(index,1);
}
if(wordList.length == 1){
    wordList = wordList.concat(usedWords);
    usedWords = []
}

localStorage.setItem('words', JSON.stringify(wordList)); 
const answer=wordList[random];
let usedWords=[];
let alphabets = [
    'A', 'B', 'C', 'D', 'E', 'F',
    'G', 'H', 'I', 'J', 'K', 'L', 'N', 'M',
...
window.addEventListener('keydown', (e) => {
    if (alphabets.includes(e.key.toUpperCase()) && gameStatus == 'playing') { // check for letters 
        ...
        if (currentLetter == 5 && gameStatus == "playing" && document.getElementById(currentInputId).value != '') {
            ...
            if (word == answer.toUpperCase()) { // check if the word is correct
                ...
                letters.forEach(i => {
                    ...
                    if (input.value == answer[i-1].toUpperCase()) { // letter exists at correct position
                        ...
                        if (answer.toUpperCase().split("").includes(input.value)) { // letter exists but not at correct position
